...
import {animationFrame,copyObj} from "./utils.js";
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const wrap = templateRef(
        `wrap${classOption["key"]}`,
        null
    );
    const slotList = templateRef(
        `slotList${classOption["key"]}`,
        null
    );
    const realBox = templateRef(
    ...
    let defaultOption = computed(() => {
      return {
        ...
        step: 1,
        //启动无缝滚动最小数据数
        limitMoveNum: props.minLimitMoveNum,
        //是否启用鼠标hover控制
        hoverStop: true,
